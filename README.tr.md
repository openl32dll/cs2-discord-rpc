...
Discord'da görünen metin (mod adları, "Isınma turu" / "Bomba döşendi" gibi
durum yazıları) **varsayılan olarak İngilizce**'dir. Türkçe görmek
istersen `RPC_LANGUAGE` ortam değişkenini ya da `config.json` içindeki
`"language"` alanını `tr` yap:

```bash
RPC_LANGUAGE=tr cs2-discord-rpc
```

ya da `config.json` içinde:
```json
{
  "discord_client_id": "BURAYA_DISCORD_CLIENT_ID_YAZ",
  "language": "tr"
}
```

Tanınmayan bir değer verilirse otomatik olarak İngilizce'ye düşer. Şu an
sadece `en` ve `tr` destekleniyor; başka bir dil eklemek istersen
`cs2_discord_rpc.py` içindeki `STRINGS` sözlüğüne katkı sağlayabilirsin.
